const linearSearch = (array, element) => {
    for (let i = 0; i < array.length; i++) {
        if (array[i] === element) return i
    }
    return -1
}

const linearSearchRecursive = (array, element, index = 0) => {
    if (index > array.length) return -1
    else if (element === array[index]) return index
    return linearSearch(array, element, index += 1)
}

const binarySearch = (array, element) => {
    let low = 0
    let high = array.length - 1
    let sortedArray = array.sort((a, b) => a - b)

    while (low <= high) {
        let mid = Math.floor((high + low) / 2)

        if (sortedArray[mid] === element) return mid

        if (element > array[mid]) low = mid + 1

        else high = mid - 1

    }

    return -1
}

const binarySearchRecursive = (array, val) => {
    sortedArray = array.sort((a, b) => a - b);
    return binarySearchRecursiveUtil(sortedArray, val, sortedArray.length, 0);
}

const binarySearchRecursiveUtil = (array,val,high = array.length,low = 0) =>{

    let mid = Math.floor((high+low)/2)
    
    if(array[mid] === val) return mid
    
    if(low >= high) return -1
    
    if(val > array[mid]) low = mid + 1
    
    else{
        high = mid -1
    }
    
    return binarySearchRecursiveUtil(array,val,low, high)
    }


binarySearchRecursive([1, 2, 3, 4], 4)